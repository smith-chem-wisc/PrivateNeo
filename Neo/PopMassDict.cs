    ...
    class PopMassDict
    {
        ...
        public void PopulateMassDictionary()
        {
            ...
            /*         List<char> firstAA = new List<char>();
                     foreach(char aa in Neo.AANames)
                     {
                         firstAA.Add(aa);
                     }
                     Parallel.ForEach(Partitioner.Creat(0,))*/
            while (indexes[0] < Neo.AANames.Count())
            {
                //get new seq
                seq = "";
                if (indexes[2] == 15)
                ...
                if (fragMass < maxMass)
                {
                    ...
                    {
                        if (indexes[length - 1] < Neo.AANames.Count() - 1) //if not last aa in possible aa
                        {
                            indexes[length - 1]++;
                        }
                ...
                {
                    if (indexes[i] == Neo.AANames.Count())
                    {
                        if (i > 0)
                        {
                            length--;
